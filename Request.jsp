...
<th style="text-align:center; color:#00fc0d; font-family: The Devil Net; font-size: 19px;">STATUS</th>
   ...
   String hosp=""; 
	...
	Connection con=null;
    con=Dbcon.create();
	PreparedStatement ps=con.prepareStatement("SELECT Dname,Lname,Disease, `Dis attacked early` FROM secure.patientdett;");
    ...
    con1=Dbcon.create();
	PreparedStatement ps1=con1.prepareStatement("SELECT cspkey FROM secure.kkregg");
...
<td align="left"><a href="Request?doctor=<%=dname%>&&patient=<%=pname%>&&disease=<%=disname%>
&&hosp=<%=hosp%>&&cspkey=<%=rs1.getString(1)%>"><button>REQUEST</button></a></td>
